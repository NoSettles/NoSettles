...
$ptid = "";
$space = "";
$sql = "SELECT ptid FROM enroll WHERE username='$u' ORDER BY date DESC LIMIT 10";
$query = mysqli_query($db_conx, $sql);
$statusnumrows = mysqli_num_rows($query);
if($statusnumrows > 0){
	...
	$statuslist = '<div class="med-large-font font-lightgray center-text full-width">'.$firstName.' has not enrolled on any sources.</div><div class="dummy-space">
	</div>';
}
...
$sql = "SELECT ptid FROM enroll WHERE username='$u' ORDER BY date DESC";
$query = mysqli_query($db_conx, $sql);
$user_results = mysqli_num_rows($query);
?>
<!DOCTYPE html>
<html>
<head>
	...
	<main>
		<div class="parallax" style="padding-bottom: 0;">
			...
			<div class="parallax-user">
				...
				<div class="pageMiddle med-margin-top relative user-results" style="z-index: 20;">
					<div class="display-block center-text full-width sourcenav">
						<a href="user.php?u=<?php echo $u; ?>" class="user-nav no-underline border-blue med-padding large-padding-left large-padding-right blue-background font-white">Enrolled Skills </a>
						<a href="post.php?u=<?php echo $u; ?>" class="user-nav no-underline border-blue med-padding large-padding-left large-padding-right">Sources Shared </a>
					</div>

					<h2 class="large-font center-text font-lightgray"><?php echo $firstName; ?>'s Learning... </h2>
...
$(function() {
	...
	$(window).scroll(function() {
		...
		if(sc >= dh - (wh + 400) ) {
			if(post_nm < +'<?php echo $user_results ?>'){
